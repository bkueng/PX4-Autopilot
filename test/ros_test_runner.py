...
def main() -> NoReturn:
    ...
    parser.add_argument("--px4-sdk-build-dir", type=str,
                        default=None,
    ...
    if ros_package_build_dir is None:
        ros_package_build_dir = lookup_px4_sdk_build_dir()
        if args.verbose:
            print(f'px4_sdk_cpp build dir: {ros_package_build_dir}')
...
def lookup_px4_sdk_build_dir():
    """Get the ROS2 build directory for px4_sdk_cpp"""
    try:
        p = subprocess.Popen(
            'ros2 pkg prefix px4_sdk_cpp'.split(),
            stdin=subprocess.PIPE,
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT)
    except FileNotFoundError:
    ...
    lines = str(p.stdout.read().decode("utf-8")).strip().split('\n')
    assert len(lines) == 1
    return os.path.join(lines[0], '../../build/px4_sdk_cpp')
